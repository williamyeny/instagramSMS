...
app.post('/getsms', function(req, res) {
  ...
  if (spl[0].toLowerCase() == "reply") {
    ...
    request.post({
      ...
      formData: {comment_text: '@' + notifList[ind].username + ': ' +  bod.slice(spl[1].length + 6, bod.length)}, //adding the username of the requester increases question variability which helps hide from the spam filter
  ...
  twiml.message('message successfully sent: ' + bod.slice(spl[1].length + 6, bod.length));
